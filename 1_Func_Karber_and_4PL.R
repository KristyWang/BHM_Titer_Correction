...
# ELISA: Function for calculating individual endpoint titers using 4PL model ---- 
DRM_function_ELISA <- function(data, 
                               sam_id_var_name = "sam_id", 
                               bat_id_var_name = "bat_id",
                               dilution_var_name = "dilution", 
                               OD_var_name = "OD",
                               min_dilution = 200,
                               max_dilution = 3276800,
                               dilution_interval = 4,
                               cut_off = 0.1){
  ...
  data$bat_id <- data[, which(colnames(data) == bat_id_var_name)]
  data$dilution <- data[, which(colnames(data) == dilution_var_name)]
  data$OD <- data[, which(colnames(data) == OD_var_name)]
  
  max_y <- data %>% filter(dilution == min_dilution) %>% 
    group_by(sam_id, bat_id) %>%
    summarise(max_y = mean(OD))
  
  neg_id <- max_y %>% filter(max_y < cut_off)
  pos_id <- data %>% filter(!sam_id %in% neg_id$sam_id) %>% 
    group_by(sam_id, bat_id) %>% slice(1)
  ...
  for (j in unique(pos_id$sam_id)){
    ...
    for (k in unique(sample_j$bat_id)){
      ...
      bat_k$dilution <- as.numeric(as.character(bat_k$dilution))
      tmp <- tryCatch({
        model <- drm(data = bat_k, mean_OD ~ dilution, 
                     fct = LL.4())
        summary(model)
        result <- summary(model)$coefficients[, 1]
        b <- result[1]
        ...
        e <- log(result[4])
        if ((d <= cut_off) | (b <= 0)){
          EP = log10(min_dilution) - log10(dilution_interval)
        } else if (c > cut_off){
          EP = log10(max_dilution)
        } else{
          EP = pmax(log10(exp(log((d-c)/(cut_off-c)-1)/(b) + e)), log10(min_dilution) - log10(dilution_interval))
          EP = pmin(EP, log10(max_dilution))
        }
  ...
  tmp$d <- NA
  tmp$e <- NA
  tmp$EP.4pl <- log10(min_dilution) - log10(dilution_interval)
  colnames(tmp) <- colnames(DRM_data)
  DRM_data <- rbind(DRM_data, tmp)
