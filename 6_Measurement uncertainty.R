...
summary_VC <- data_VC %>% group_by(vd, batch) %>% summarise(n=n())
...
summary_control <- data %>%
  filter(dilution == 120 & Serum_id %in% c("PC","IS500","IS1000")) %>%
  group_by(Serum_id, batch, vd, stock) %>%
  summarise(n=n())

Basic_info <- data.table(read.csv("../1. Data/Basic_info.csv"))[, c(1,2,3,4,5,6,10)]
sample_data <- data %>% filter(!Serum_id %in% c("PC","IS500","IS1000")) %>% 
  group_by(Serum_id, vd, stock, batch) %>% summarise(n = n())
sample_data <- data.table(sample_data)
sample_data[Serum_id == 60538, Serum_id := "6053C8"]
sample_data[Serum_id == 62878, Serum_id := "6287C8"]
sample_data[Serum_id == 62128, Serum_id := "6212C8"]
sample_data <- data.table(left_join(sample_data, Basic_info))
sample_data <- sample_data[!is.na(age), ]
sample_data[Serum_id %in% c("6053C8", "6287C8", "6212C8"), ]$Visit <- "CRS"
summary_sample <- sample_data %>% group_by(batch, vd, stock, source) %>% summarise(n = n())

rm(sample_data, Basic_info)
rm(summary_control, summary_VC, summary_sample)
...
plot_data %>% filter(Method == "Karber Method") %>% ggplot() +
  ...
  annotate("text", x = 6.5, y = 0.64, label = "Kärber Method", size = 3, vjust = 1, hjust = 0, fontface = "bold") +
  theme_bw() +
