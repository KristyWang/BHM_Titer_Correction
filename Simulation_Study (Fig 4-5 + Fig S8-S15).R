...
# 4. Figures ----
## Fig. S8 effects of virus working stock ----
...
tiff("Fig_S8.tiff", width = 8, height = 2.5, units = "in", res = 300)
ggplot() +
...
tiff("Fig_S10.tiff", width = 8, height = 2.5, units = "in", res = 300)
...
### Fig. S11 batch-level parameters ----
set.seed(123)
selected_batches <- sample(unique(f_bat_true$bat_id), 6)

tiff("Fig_S11.tiff", width = 12, height = 6, units = "in", res = 300)
ggplot() +
  scale_color_manual("",values = c("Prior" = "#6BAED6", "Posterior" = "#D53E4F", "True" = "black")) +
  scale_fill_manual("", values = c("Prior" = "#9ECAE1", "Posterior" = "#FDAE61", "True" = "black")) +
  facet_grid(
...
### Fig. S12 sample-level parameters ----
tiff("Fig_S12.tiff", width = 12, height = 6, units = "in", res = 300)
ggplot() +
  scale_color_manual("",values = c("Prior" = "#6BAED6", "Posterior" = "#D53E4F", "True" = "black")) +
  scale_fill_manual("", values = c("Prior" = "#9ECAE1", "Posterior" = "#FDAE61", "True" = "black")) +
  facet_grid(
...
## Fig. S13 Agreement in batch- and sample-level parameters ----
tiff("Fig_S13.tiff", width = 4.5, height = 6, units = "in", res = 300)
ggarrange(agreement_batch, agreement_sample, nrow=2, align = "hv",
          labels = c("A", "B"))
dev.off()

## Fig. 5: Agreement in titer estimates ----
...
#### 22 negatives 
df_neg <- df[df$Nt_true <= log2(40), ]
df_neg %>% mutate(diff_Karber = Nt.Karber - Nt_true,
                  diff_4PL = Nt.4pl - Nt_true,
                  diff_BHM = Nt.adj - Nt_true) %>%
  summarise(GMT.Karber = mean(Nt.Karber),
            GMT.4PL = mean(Nt.4pl),
            GMT.BHM = mean(Nt.adj),
            RMSE.Karber = sqrt(mean(diff_Karber^2)),
            RMSE.4PL = sqrt(mean(diff_4PL^2)),
            RMSE.BHM = sqrt(mean(diff_BHM^2)))
...
ggplot(df_diff, aes(x = Method, y = diff, color = Method, fill = Method)) +
  ...
  scale_x_discrete("Methods", labels = c("Kärber\nMethod",
                                         "4PL\nModel",
                                         "BHM\nUnadjusted",
                                         "BHM\nAdjusted")) +
...
# Output
tiff("Fig_5.tiff", width = 9, height = 5 , units = "in", res = 600)
...
## Fig. 4: illustrate simulated data ----
...
# Output
tiff("Fig_4.tiff", width = 7.5, height = 5.5 , units = "in", res = 600)
l1 <- ggarrange(p1, p2, nrow = 1, widths = c(1.35, 1), 
                labels = c("A ", "B "),
                font.label = list(size = 10))
ggarrange(l1, p3, nrow = 2, labels = c("", "C "), 
          heights = c(1, 1), font.label = list(size = 10))
dev.off()

## Fig. S15 4PL false negatives ----
...
# Output
tiff("Fig_S15.tiff", width = 10, height = 6 , units = "in", res = 600)
p4
dev.off()

## Fig. S14 Pred. v.s. true -----
# PC
...
tiff("Fig_S14.tiff", width = 8.7, height = 7, units = "in", res = 300)
...
tiff("Fig_S9.tiff", width = 12, height = 10, units = "in", res = 300)
